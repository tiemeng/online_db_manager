                        ...
                        @foreach($datas as $k=>$item)
                            ...
                            @else
                                <tr class="b-group">
                                    <th width="10%">
                                        <label>
                                            @if($item->route == 'index.index')
                                                {{$item->name}}&nbsp;<input type="checkbox" name="rule_id[]" value="{{$item->id}}"  checked="checked"  onclick="return false">
                                            @else
                                        ...
                                        @foreach($item->_data as $key=>$value)
                                            <table class="table table-striped table-bordered table-hover table-condensed">
                                                <tr class="b-group">
                                                    <th width="10%">
                                                        <label>
                                                            @if($value->route == 'index.main')
                                                                {{$value->name}}&nbsp;<input type="checkbox" name="rule_id[]" value="{{$value->id}}"  checked="checked"  onclick="return false">
                                                            @else
