...
use App\Models\Deposit\Payments;
...
class Payment extends Command
{
     ...
     * @var string
     */
    protected $signature = 'payment:done';
     ...
     * @var string
     */
    protected $description = 'payment migration';
    ...
    protected function _insert(string $db,string $connection = 'deposit')
    {
        $len = true;
        $table = 'payments';
        $payment_model = (new Payments())->setConnection($connection);
        while ($len){
            $last_id = SyncRecords::getLastId($db,$table);
            $list = $payment_model->getPayments($last_id);
            if($list){
                $payment_last_id = $payment_model->getMaxId($last_id,$connection);
                if(\App\Models\Payments::insert($list)){
                    //失败重试一次
